...
class ETF(Strategy):
    def __init__(self, symbol: str, holdings, threshold):
        super().__init__(symbol)
    ...
    def synthetic_price(self, state: TradingState):
        ...
        for product, quantity in self.holdings.items():
            if product in state.order_depths:
                ...
                price += quantity * self.vwap(order_depth)
            ...
            for product, quantity in self.holdings.items():
                if product in state.order_depths:
                    orders.append(
                        Order(
                            product,
                            self.vwap(state.order_depths[product]),
                            -quantity * size,
                        )
                    )
    ...
    def run(self, state: TradingState):
        ...
        if not state.traderData:
            ...
            traderData["PICNIC_BASKET1"] = ETF(
                "PICNIC_BASKET1", {"CROISSANTS": 6, "JAMS": 3, "DJEMBES": 1}, 120
            )
            traderData["PICNIC_BASKET2"] = ETF(
                "PICNIC_BASKET2", {"CROISSANTS": 4, "JAMS": 2}, 100
            )
        else:
