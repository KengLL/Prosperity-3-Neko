...
class Coupons(Strategy):
    ...
    def iv_smile(self, moneyness):
        # Assuming a simple quadratic smile for demonstration purposes
        # This can be replaced with a more complex model if needed
        coeffs = [0.21100771, 0.00137945, 0.15031193]
        return np.polyval(coeffs, moneyness)
    ...
    def run(self, state):
        ...
        underlying_price = self.vwap(state.order_depths[self.underlying])
        option_price = self.vwap(state.order_depths[self.symbol])
        underlying_position = (
            state.position[self.underlying] if self.underlying in state.position else 0
        )

        if underlying_price == 0 or option_price == 0:
            return orders
        moneyness = math.log(self.strike / underlying_price) / math.sqrt(self.expiry)
        iv = self.implied_volatility(
            option_price, underlying_price, self.strike, self.expiry
        )
        fit_iv = self.iv_smile(moneyness)

        diff = option_price - self.black_scholes_call(
            underlying_price, self.strike, self.expiry, fit_iv
        )
        self.history.append(diff)
        ...
        if len(self.history) > self.window:
            ...
            # logger.print(f"{self.symbol[-5:]} delta: {z_score:.2f}")
            if z_score > self.threshold:
                orders.append(
                    Order(self.symbol, option_price, -self.limit - self.position)
                )
                ...
                orders.append(Order(self.symbol, option_price, -self.position))
...
class Rock(Strategy):
    def __init__(self, symbol: str, window, threshold):
        super().__init__(symbol)
        self.position = 0
        self.limit = 400
        self.window = window
